...
| ------------------------ | ------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `colors`                 | string[] or Promise<string[]> or object[] or Promise<object[]> or ColorGroup[] or Promise<ColorGroup[]> | null    | Array of colors to be displayed in the palette. Pass an array of `{ name, colors }` objects to display grouped collections. Supports `bind:colors`: internal add/delete mutations write the whole resolved list back — normalized `{ value, name? }` colors, or, in grouped mode, the groups as they were supplied with only their colors normalized, so the groups the grid skips and the keys they carry survive — including the entries `maxColors`, `allowDuplicates` or compact mode withhold from the grid (a passed `Promise` or `string[]` is replaced by that value). A supplied group list mutated in place rather than reassigned drifts out of step with the palette, and the resolved groups are written back in rendered order instead. Omitting `colors` (or passing `null`) leaves the palette in its loading state — this is the intended behaviour while an async source is pending; pass `[]` explicitly to render an empty palette. See more about colors in the [Colors Setting](#colors-setting) section |
...
| `isCompact`              | boolean                                                                                                 | false   | Flag to display the palette in compact mode. Grouped colors compact too: a compact palette ignores the grouping and renders the picked colors as a single flat strip (see [Compact Mode](#compact-mode)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `compactColorIndices`    | number[]                                                                                                | []      | Array of indices to pick from the `colors` array to be displayed in the compacted palette (see [Compact Mode](#compact-mode)). With a grouped `colors` list the indices address the groups' colors flattened into one list, in group order — not the list `ondelete` resolves against, which reports the color's index in its own full group along with that group's `groupIndex`. Supports `bind:compactColorIndices`: deleting a slot from the flat or the compact palette re-indexes it, and drops any index whose color the deletion removed as well as any the shortened list no longer reaches.                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `allowDuplicates`        | boolean                                                                                                 | false   | Flag to allow color duplication. When `false` the palette renders one slot per distinct color — per group in grouped mode — and deleting that slot removes every entry it stands for: every occurrence in the list, in its group in grouped mode, or within the [`compactColorIndices`](#compact-mode) selection in compact mode — across every group there, since a compact palette flattens its groups first, so a color repeated in two groups renders one slot and deleting it reaches both (see [Deletion Modes](#deletion-modes)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `showTransparentSlot`    | boolean                                                                                                 | false   | Flag to display a transparent slot at the start of the slot list. It is not rendered on a grouped `colors` list, whether the groups are displayed or the palette is compact, so collapsing a grouped palette never adds one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `maxColors`              | number                                                                                                  | 30      | Maximum number of slots to be displayed in the palette. Set this value to `-1` to allow infinite number of slots. In grouped mode the cap applies to each group rather than to the palette. While that palette is compact the strip is one flat list, so the cap applies to the whole strip instead. The palette never grows its own list past the cap: once the rendered slots are full the input refuses the submission and nothing changes — no slot, no write-back, no `onadd`. A list supplied above the cap is left as it is, so the colors beyond the cap stay in the list and are handed back to `bind:colors` and to `ondelete`; deleting a rendered slot pulls the next one into view, and the input accepts again once the list itself drops below the cap.                                                                                                                                                                                                                                                         |
...
| `numColumns`             | number                                                                                                  | 5       | Number of columns of the palette grid. Set this value to `0` to display the slots on a single row (see `maxColumns`). Values lower than `0` are treated the same as `0`. A positive value is floored to a whole number and held at a minimum of `1`, so `numColumns={2.5}` renders two columns. In grouped mode it is the width of every group: a group holding more colors wraps onto further rows. `0` sizes the grid to the longest group, subject to the same five-column minimum as flat mode and to `maxColumns`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `onadd`    |              |                          | **Called once a color has been added to the list through the input.** The palette refuses a submission that repeats a color already in the list when `allowDuplicates` is `false`, and any submission made once the rendered slots are full; a refused submission adds nothing, writes nothing back and fires no callback.                                                                                                                                                                     |
|            | `color`      | string                   | The color value that was added.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|            | `colors`     | object[] or ColorGroup[] | The resulting color list, in its resolved and normalized form, holding every color the palette carries, including the ones the rendered slots withhold through `allowDuplicates` or compact mode.                                                                                                                                                                                                                                                                                              |
...
|            | `index`      | number                   | Index of the removed color in the resolved full list, or in its full group in grouped mode — never the index of the rendered slot. With `allowDuplicates` off, one deletion drops every occurrence and this reports the first — the first held by `compactColorIndices` in compact mode. From a compact strip over grouped colors it still addresses the full group the deleted slot resolves to, which `groupIndex` and `groupName` name.                                                     |
|            | `colors`     | object[] or ColorGroup[] | The resulting color list, in its resolved and normalized form, holding every color the palette carries, including the ones the rendered slots withhold through `maxColors`, `allowDuplicates` or compact mode. In grouped mode it is the supplied group list with only its colors normalized. A supplied list that has drifted out of step — its groups mutated in place rather than reassigned — is replaced by the resolved groups, in rendered order and without the groups the grid skips. |
|            | `groupIndex` | number                   | Index of the group the color was removed from, in the supplied list rather than in the rendered one (grouped mode only). It always addresses the list `colors` carries, so it reports the rendered index whenever that list falls back to the resolved groups.                                                                                                                                                                                                                                 |
...
| `slot`            | Allow to replace the default color slots. `groupName` is only set while the groups are displayed, never while the palette is compact. Compose [`PaletteSlot`](#paletteslot). | `index`, `color`, `colorName`, `groupName`, `selectedColor`, `selected`, `transition`, `isCompact`, `tabindex`, `ariaKeyShortcuts` |
| `transparentSlot` | Allow to replace the default transparent slot. Never rendered on a grouped `colors` list, compact or not.                                                                    | `tabindex`, `selected`                                                                                                             |
| `beforeSlot`      | Allow to add an element before the color slots. Never rendered on a grouped `colors` list, compact or not.                                                                   | `selectedColor`, `transition`, `isCompact`                                                                                         |
| `afterSlot`       | Allow to add an element after the color slots. Never rendered on a grouped `colors` list, compact or not.                                                                    | `selectedColor`, `transition`, `isCompact`                                                                                         |
...
When groups are used the color input is not available. Compact mode is: collapsing a grouped palette ignores the grouping and renders one flat strip of the picked colors — no group names, no per-group grids, no `group` roles and no `groupName` on the `slot` snippet — and enlarging restores the groups untouched (see [Compact Mode](#compact-mode)). While the groups are displayed the `slot` snippet receives an additional `groupName` parameter.
...
Whichever mode is used, the gesture deletes from the resolved `colors` list rather than from the rendered slots: `ondelete` reports the removed color's index in that full list — in its full group in grouped mode, the group itself reported by its index in the supplied list — and hands the whole list back. When that supplied list has drifted out of step with the palette, mutated in place rather than reassigned, the resolved groups are handed back in rendered order instead and the reported group index follows them. With `allowDuplicates` left at its `false` default a rendered slot stands for every entry that repeats its color, so deleting the slot drops all of them: every occurrence in the list, in the group in grouped mode, or within the [`compactColorIndices`](#compact-mode) selection in compact mode. The match is case-insensitive, so `#AABBCC` and `#aabbcc` go together. Set `allowDuplicates` to `true` to give every entry its own slot and delete exactly one per gesture. On a compact grouped palette that selection spans the whole flattened list, so one gesture can reach into more than one group; `ondelete` reports the group the deleted slot itself resolves to and hands the whole group list back.
...
Deleting a compact slot removes the mapped color from the underlying full `colors` list (not just from the compact view), fires `ondelete` with that color's index in the full list, and re-indexes `compactColorIndices`. With `allowDuplicates` left at `false` it drops every occurrence the compact selection holds, leaving any occurrence outside that selection in place. Deleting a slot from the expanded palette re-indexes `compactColorIndices` the same way, so the compact subset still points at the same colors once the palette is collapsed again; an index whose color the deletion removed is dropped, as is one the shortened list no longer reaches, and when that empties the subset the compact toggle goes with it. Both `colors` and `compactColorIndices` support `bind:` so the mutation can be persisted; pass a stable reference (or bind it) for `compactColorIndices` so the re-index is not overwritten on the next render. The same re-indexing and duplicate-dropping rules apply to a grouped palette, over the flattened list, whether the slot is deleted from the compact strip or from the expanded groups — the reported `ondelete` index does not follow that flattened list, though: it stays relative to the group the color belongs to (see [Grouped Palettes](#grouped-palettes)).

### Grouped Palettes

Compact mode ignores the grouping. A grouped `colors` list collapses into one flat strip rendered through the same markup as a flat palette — the group names, the per-group `group` roles and the per-group grids are gone while the palette is compact — and enlarging restores the groups. The strip adds no slot the grouped display does not have: `showTransparentSlot`, `beforeSlot` and `afterSlot` stay out of a grouped palette in both states, so collapsing it narrows the colors on show without introducing anything around them.

`compactColorIndices` addresses the groups' colors flattened into a single list, in group order. `allowDuplicates` and `maxColors`, which apply per group in the grouped display, apply to that one strip while the palette is compact: a color held by two groups renders a single slot there where the expanded palette renders one per group.
...
<script>
	...
	const compactColorIndices = [0, 4]
</script>

<Palette {colors} {compactColorIndices} />
```

Index `0` is `Warm`'s `#865C54` and index `4` is `Cool`'s `#32465C`.

Deleting a slot from the strip removes the color from the group that holds it: `ondelete` reports the color's index in that group's full colors along with the group's `groupIndex` and `groupName`, `bind:colors` is written back as groups, and `compactColorIndices` is re-indexed over the flattened list. With `allowDuplicates` left at `false`, a slot standing for the same color in two groups drops both — every occurrence the selection holds, in whichever group it sits.
...
When colors are grouped, each group keeps its own `.palette__cells` grid instead — laid out on the same `--num-columns`, so a group holding more colors than the column count wraps onto further rows. While that palette is compact the groups collapse into the flat `.palette__listbox` grid above, so a rule targeting `.palette__groups__group > ul.palette__cells` does not reach the compact strip. That grid is declared inside `:where()`, at zero specificity, so any ordinary rule of yours replaces it without `!important` and without borrowing the palette's own class names:
