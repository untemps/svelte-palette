...
| ------------------------ | ------------------------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `colors`                 | string[] or Promise<string[]> or object[] or Promise<object[]> or ColorGroup[] or Promise<ColorGroup[]> | null    | Array of colors to be displayed in the palette. Pass an array of `{ name, colors }` objects to display grouped collections. Supports `bind:colors`: internal add/delete mutations write the whole resolved list back — normalized `{ value, name? }` colors, or `{ name?, colors }` groups in grouped mode — including the entries `maxColors`, `allowDuplicates` or compact mode withhold from the grid (a passed `Promise` or `string[]` is replaced by that value). Omitting `colors` (or passing `null`) leaves the palette in its loading state — this is the intended behaviour while an async source is pending; pass `[]` explicitly to render an empty palette. See more about colors in the [Colors Setting](#colors-setting) section |
...
Whichever mode is used, the gesture deletes from the resolved `colors` list rather than from the rendered slots: `ondelete` reports the removed color's index in that full list — in its full group in grouped mode — and hands the whole list back. With `allowDuplicates` left at its `false` default a rendered slot stands for every entry that repeats its color, so deleting the slot drops all of them: every occurrence in the list, in the group in grouped mode, or within the [`compactColorIndices`](#compact-mode) selection in compact mode. The match is case-insensitive, so `#AABBCC` and `#aabbcc` go together. Set `allowDuplicates` to `true` to give every entry its own slot and delete exactly one per gesture.
