...
| `compactColorIndices`    | number[]                                                                                                | []      | Array of indices to pick from the `colors` array to be displayed in the compacted palette (see [Compact Mode](#compact-mode)). Supports `bind:compactColorIndices`: deleting a slot from the flat or the compact palette re-indexes it, and drops any index whose color the deletion removed as well as any the shortened list no longer reaches.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `allowDuplicates`        | boolean                                                                                                 | false   | Flag to allow color duplication. When `false` the palette renders one slot per distinct color — per group in grouped mode — and deleting that slot removes every entry it stands for: every occurrence in the list, in its group in grouped mode, or within the [`compactColorIndices`](#compact-mode) selection in compact mode (see [Deletion Modes](#deletion-modes)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `maxColors`              | number                                                                                                  | 30      | Maximum number of slots to be displayed in the palette. Set this value to `-1` to allow infinite number of slots. In grouped mode the cap applies to each group rather than to the palette. The palette never grows its own list past the cap: once the rendered slots are full the input refuses the submission and nothing changes — no slot, no write-back, no `onadd`. A list supplied above the cap is left as it is, so the colors beyond the cap stay in the list and are handed back to `bind:colors` and to `ondelete`; deleting a rendered slot pulls the next one into view, and the input accepts again once the list itself drops below the cap.                                                                                                                                                                                                                                                                                                                                                                  |
...
|            | `index`      | number                   | Index of the removed color in the resolved full list, or in its full group in grouped mode — never the index of the rendered slot. With `allowDuplicates` off, one deletion drops every occurrence and this reports the first — the first held by `compactColorIndices` in compact mode.                                                                                                                                                                                                       |
